    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-[#0a0a0a] border-l-4 border-[#39FF14] z-[101] transition-transform duration-500 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col shadow-[-20px_0_50px_rgba(57,255,20,0.2)]`}>
        <div className="p-6 border-b border-white/10 flex justify-between items-center">
          <h2 className="text-4xl heading-font neon-text-green">Your Stash</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
              ...
              <p className="text-xl body-font text-gray-400">Your stash is empty, fam.</p>
              <button 
                onClick={onClose}
            ...
            items.map((item) => (
              <div key={item.id} className="flex gap-4 bg-white/5 p-4 rounded-2xl border border-white/10 group">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[#39FF14] font-bold mb-2">${item.price}</p>
        ...
        {items.length > 0 && (
          <div className="p-8 bg-[#111] border-t border-white/10 space-y-4">
            <div className="flex justify-between text-2xl heading-font">
              <span>Total</span>
              <span className="neon-text-yellow">${total.toFixed(2)}</span>
            </div>
            <button className="w-full py-5 bg-[#BC13FE] text-white font-black text-2xl rounded-2xl shadow-[4px_4px_0px_0px_#fff] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all uppercase">
              Secure The Bag
            </button>
            <p className="text-[10px] text-center text-gray-500 uppercase tracking-widest">
              Standard delivery • 21+ ID Required
            </p>
          </div>
        )}
      </div>
