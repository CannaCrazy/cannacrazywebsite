...
/* ==============================================
   BACKGROUND EFFECTS (Shimmer & Particles)
   ============================================== */
...
const cannaCrazyCart = {
    ...
    showNotification: function (message) {
        ...
        notification.style.cssText = `
            ...
            color: black; padding: 1rem 1.5rem; border-radius: 1rem;
            font-weight: bold; box-shadow: 0 10px 30px rgba(57, 255, 20, 0.3);
                ...
                <div>
                    ...
                    ${product.effects && product.effects.length > 0 ? `
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">${effectsHTML}</div>
                        </div>
                    ` : ''}
    ...
    toggleMobileMenu: function () {
        ...
        if (nav) {
            ...
            nav.style.cssText += `
                ...
                background-color: #0a0a0a; flex-direction: column;
...
const cannaCrazyBudtender = {
    ...
    openModal: function () {
        ...
        if (modal) {
            ...
            document.getElementById('budtender-input')?.focus();
        }
    },

    closeModal: function () {
        document.getElementById('budtender-modal')?.classList.remove('active');
    ...
    createModal: function () {
            ...
            <div class="modal-overlay" id="budtender-modal">
                <div class="modal-content" style="max-width: 700px; max-height: 80vh; display: flex; flex-direction: column;">
                    ...
                    <div style="padding: 2rem; border-bottom: 1px solid rgba(255,255,255,0.1);">
                        ...
                        <p style="color: #9ca3af; margin-top: 0.5rem;">Tell me what you're looking for and I'll recommend products from our current inventory!</p>
                    </div>
                    ...
                    <div style="padding: 1.5rem; border-top: 1px solid rgba(255,255,255,0.1);">
                        <form id="budtender-form" style="display: flex; gap: 0.75rem;">
                            <input type="text" id="budtender-input" placeholder="E.g., Something to help me relax after work..." required style="flex: 1; padding: 0.75rem 1rem; border-radius: 0.75rem; background-color: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white; font-size: 1rem;" />
                            <button type="submit" id="budtender-submit" style="padding: 0.75rem 1.5rem; background: linear-gradient(135deg, #BC13FE, #39FF14); color: black; border: none; border-radius: 0.75rem; font-weight: bold; cursor: pointer; transition: all 0.3s ease;">Send</button>
                        </form>
                        <p style="font-size: 0.75rem; color: #6b7280; margin-top: 0.75rem; text-align: center;">Powered by Google Gemini AI • Grounded in our live inventory</p>
                    </div>
                </div>
            </div>
            <style>
                ...
                #budtender-submit:hover { transform: scale(1.05); }
                #budtender-submit:disabled { opacity: 0.5; cursor: not-allowed; }
        ...
        document.getElementById('budtender-form')?.addEventListener('submit', (e) => {
            e.preventDefault();
            this.sendMessage();
        });
    },
    ...
    addMessageToChat: function (type, message) {
        ...
        messageDiv.innerHTML = type === 'ai'
            ? `<strong style="color: #39FF14;">AI Budtender:</strong><p>${this.formatMessage(message)}</p>`
            : `<strong style="color: #BC13FE;">You:</strong><p>${message}</p>`;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Budtender is initialized on button click usually, or just ready to go
